...
async def admin_api_manager(callback: CallbackQuery, state: FSMContext):
    ...
    kb = InlineKeyboardMarkup(inline_keyboard=[
        [InlineKeyboardButton(text="🛒 Connect Shop API", callback_data="admin:shop_api_connect", style="success")],
        ...
        [InlineKeyboardButton(text="➕ Add Generic API Provider", callback_data="admin:api_add", style="primary")],
        [InlineKeyboardButton(text="◀ Back", callback_data="admin:home", style="danger")],
    ])
    await edit(callback, "\n".join(lines), kb)
...
async def main():
    ...
    init_db()
    migrate_v23_schema()
    # V25.6 changes Shop API import from automatic to admin-selected only.
