# Premium Hub Store Bot - V25.6 Manual Shop API Products
# Python 3.10+
# Dependencies:
...
# WEB_HOST=0.0.0.0
# WEB_PORT=8080
# Optional Brevo email API (recommended on Railway):
# BREVO_API_KEY=xkeysib-...
# BREVO_SENDER_EMAIL=verified-sender@example.com
# BREVO_SENDER_NAME=Premium Hub Store
...
# Brevo HTTPS Transactional Email API (preferred on Railway; no SMTP port needed)
BREVO_API_KEY = os.getenv("BREVO_API_KEY", "").strip()
BREVO_SENDER_EMAIL = os.getenv("BREVO_SENDER_EMAIL", SMTP_FROM or SMTP_USER).strip()
BREVO_SENDER_NAME = os.getenv("BREVO_SENDER_NAME", BOT_NAME).strip() or BOT_NAME
...
SHOP_API_SYNC_SECONDS = max(30, int(os.getenv("SHOP_API_SYNC_SECONDS", "60") or 60))
...
class AdminShopAPIState(StatesGroup):
    config = State()
    product_setup = State()
    ...
    with db() as con:
        if use_postgres():
            con.executescript(
                ...
                CREATE TABLE IF NOT EXISTS api_product_map (
                    local_product_id INTEGER PRIMARY KEY,
                    ...
                    cost_price TEXT,
                    profit_percent TEXT NOT NULL DEFAULT '0',
                    last_sync_at TEXT NOT NULL DEFAULT CURRENT_TIMESTAMP,
                    UNIQUE(provider_id, remote_product_id)
                );
                CREATE TABLE IF NOT EXISTS api_order_links (
                    local_order_id TEXT PRIMARY KEY,
                    provider_id INTEGER NOT NULL,
                    remote_order_code TEXT,
            ...
            con.execute("ALTER TABLE api_providers ADD COLUMN IF NOT EXISTS provider_type TEXT NOT NULL DEFAULT 'generic'")
            con.execute("ALTER TABLE api_product_map ADD COLUMN IF NOT EXISTS selected INTEGER NOT NULL DEFAULT 0")
            con.execute("ALTER TABLE api_product_map ADD COLUMN IF NOT EXISTS cost_price TEXT")
            con.execute("ALTER TABLE api_product_map ADD COLUMN IF NOT EXISTS profit_percent TEXT NOT NULL DEFAULT '0'")
            con.commit()
        ...
        api_columns = {row["name"] for row in con.execute("PRAGMA table_info(api_providers)").fetchall()}
        if "provider_type" not in api_columns:
            con.execute("ALTER TABLE api_providers ADD COLUMN provider_type TEXT NOT NULL DEFAULT 'generic'")
        map_columns = {row["name"] for row in con.execute("PRAGMA table_info(api_product_map)").fetchall()}
        if "selected" not in map_columns:
            con.execute("ALTER TABLE api_product_map ADD COLUMN selected INTEGER NOT NULL DEFAULT 0")
        if "cost_price" not in map_columns:
            con.execute("ALTER TABLE api_product_map ADD COLUMN cost_price TEXT")
        if "profit_percent" not in map_columns:
            con.execute("ALTER TABLE api_product_map ADD COLUMN profit_percent TEXT NOT NULL DEFAULT '0'")
        con.commit()
...
def refresh_runtime_settings() -> None:
    ...
    global BREVO_API_KEY, BREVO_SENDER_EMAIL, BREVO_SENDER_NAME, BREVO_API_URL
    ...
    BREVO_API_KEY = get_app_setting("brevo_api_key", BREVO_API_KEY or "").strip()
    BREVO_SENDER_EMAIL = get_app_setting(
        "brevo_sender_email", BREVO_SENDER_EMAIL or SMTP_FROM or SMTP_USER
    ).strip()
    BREVO_SENDER_NAME = get_app_setting(
        "brevo_sender_name", BREVO_SENDER_NAME or BOT_NAME
    ).strip() or BOT_NAME
    BREVO_API_URL = get_app_setting(
        "brevo_api_url", BREVO_API_URL or "https://api.brevo.com/v3/smtp/email"
    ).strip()
...
def list_products_with_stock():
    """Fetch products with local stock or the latest synced Shop API stock."""
    with db() as con:
        return con.execute(
            """
            SELECT p.*,
                   CASE WHEN m.local_product_id IS NOT NULL THEN COALESCE(m.remote_stock, 0)
                        ELSE COALESCE((SELECT COUNT(*) FROM stock_items s
                                      WHERE s.product_id=p.id AND s.status='AVAILABLE'), 0) END AS stock
            FROM products p
            LEFT JOIN api_product_map m ON m.local_product_id=p.id
            WHERE p.active=1
...
def group_products_with_stock(pid: int):
    """Fetch a standalone product/group with local or synced Shop API stock."""
    with db() as con:
        return con.execute(
            """
            SELECT p.*,
...
def stock_count(pid: int) -> int:
    with db() as con:
        mapped = con.execute(
            "SELECT remote_stock FROM api_product_map WHERE local_product_id=?",
            (pid,),
        ).fetchone()
        if mapped:
            return max(0, int(mapped["remote_stock"] or 0))
...
def soft_delete_product(pid: int) -> int:
    ...
    with db() as con:
        ...
        # For manually selected Shop API products, deleting from the shop also
        # unselects the mapping so background refresh never brings it back.
        con.execute("UPDATE api_product_map SET selected=0 WHERE local_product_id=?", (pid,))
        con.commit()
        return int(count)
...
def brevo_configured() -> bool:
    return bool(BREVO_API_KEY and BREVO_SENDER_EMAIL and BREVO_API_URL)


def email_delivery_configured() -> bool:
    """Brevo API is preferred; SMTP is retained only as an optional fallback."""
    return brevo_configured() or bool(SMTP_HOST and SMTP_USER and SMTP_PASSWORD)


async def brevo_send_email(
    to_email: str,
    subject: str,
    html_content: str,
    *,
    ...
    This works on Railway plans where outbound SMTP ports are unavailable.
    The sender email must be registered/verified in the Brevo account.
    """
    ...
    payload = {
        "sender": {
            "name": BREVO_SENDER_NAME or BOT_NAME,
    ...
    try:
        async with aiohttp.ClientSession(timeout=timeout) as session:
            async with session.post(BREVO_API_URL, headers=headers, json=payload) as resp:
                body = await resp.text()
                if resp.status not in (200, 201, 202):
                    logging.error(
                        "Brevo email failed status=%s to=%s body=%s",
                        resp.status, to_email, body[:500],
                    )
                    return False
                logging.info("Brevo email accepted to=%s status=%s", to_email, resp.status)
                return True
    except Exception:
        logging.exception("Brevo API request failed to=%s", to_email)
        return False


async def send_email_verification_code(uid: int, email: str, code: str) -> bool:
    """Send the customer's 6-digit verification code. Brevo API is preferred."""
    if not email_delivery_configured():
        logging.error("Email verification requested but no email provider is configured")
        return False
    ...
    subject = f"{BOT_NAME} • Verification Code: {code}"

    # Prefer HTTPS API on Railway. Do not fall back to blocked SMTP when Brevo is configured.
    if brevo_configured():
        return await brevo_send_email(email, subject, body)
...
async def send_invoice_email(order, bot_username: str | None = None) -> bool:
    ...
    if not user or not user["email"] or not email_delivery_configured():
        return False
    ...
    subject = f"{BOT_NAME} Invoice - {order['order_id']}"
    filename = f"invoice-{order['order_id']}.pdf"

    if brevo_configured():
        return await brevo_send_email(
            user["email"],
            subject,
            body,
...
def get_api_provider(provider_id: int):
    with db() as con:
        return con.execute("SELECT * FROM api_providers WHERE id=?", (provider_id,)).fetchone()


def add_shop_api_provider(name: str, base_url: str, api_key: str) -> int:
    with db() as con:
        cur = con.execute(
            """INSERT INTO api_providers(
                   name,base_url,api_key,auth_header,products_endpoint,balance_endpoint,
                   order_endpoint,status_endpoint,provider_type,active
               ) VALUES (?,?,?,?,?,?,?,?,?,1)""",
            (name, base_url.rstrip('/'), api_key, 'X-Shop-API-Key', '/products', '/me', '/orders', '/orders/{id}', 'shop_v1'),
        )
        con.commit()
        return int(cur.lastrowid)


def safely_remove_api_provider(provider_id: int) -> dict:
    """Disconnect an API provider without deleting customer/order/history data.

    The provider is disabled, its stored API key is erased, manually selected
    mapped products are hidden, and mappings are kept for audit/order history.
    This makes removal reversible only by adding/reconnecting the API again with
    a fresh key, while preserving all existing users, balances and orders.
    """
    provider = get_api_provider(provider_id)
    if not provider:
        raise ValueError("API provider not found.")

    hidden_products = 0
    with db() as con:
        mapped = con.execute(
            "SELECT local_product_id FROM api_product_map WHERE provider_id=?",
            (provider_id,),
        ).fetchall()
        local_ids = [int(r["local_product_id"]) for r in mapped]
        if local_ids:
            marks = ",".join("?" for _ in local_ids)
            con.execute(
                f"UPDATE products SET active=0 WHERE id IN ({marks})",
                tuple(local_ids),
            )
            hidden_products = len(local_ids)
        con.execute(
            "UPDATE api_product_map SET selected=0, remote_stock=0 WHERE provider_id=?",
            (provider_id,),
        )
        # Keep provider identity/endpoints for historical references, but erase
        # the secret and disable all future calls/background sync.
        con.execute(
            "UPDATE api_providers SET active=0, api_key='' WHERE id=?",
            (provider_id,),
        )
        con.commit()
    ...
    return {
        ...
        "name": str(provider["name"] or f"Provider {provider_id}"),
        "provider_type": str(provider["provider_type"] or "generic"),
        "hidden_products": hidden_products,
    }


def get_api_product_map(local_product_id: int):
    with db() as con:
        return con.execute(
            "SELECT * FROM api_product_map WHERE local_product_id=?",
            (local_product_id,),
        ).fetchone()


def get_api_order_link(local_order_id: str):
    with db() as con:
        return con.execute(
            "SELECT * FROM api_order_links WHERE local_order_id=?",
            (local_order_id,),
        ).fetchone()


def save_api_order_link(local_order_id: str, provider_id: int, status: str, remote_order_code: str = '', delivery=None, error: str = '', refunded: int | None = None):
    delivery_json = json.dumps(delivery or [], ensure_ascii=False) if delivery is not None else None
    with db() as con:
        existing = con.execute("SELECT * FROM api_order_links WHERE local_order_id=?", (local_order_id,)).fetchone()
        if existing:
            values = {
                'remote_order_code': remote_order_code or existing['remote_order_code'],
                'status': status or existing['status'],
                'delivery_json': delivery_json if delivery is not None else existing['delivery_json'],
                'error': error,
                'refunded': int(existing['refunded'] or 0) if refunded is None else int(refunded),
            }
            con.execute(
                """UPDATE api_order_links
                   SET remote_order_code=?, status=?, delivery_json=?, error=?, refunded=?, updated_at=CURRENT_TIMESTAMP
                   WHERE local_order_id=?""",
                (values['remote_order_code'], values['status'], values['delivery_json'], values['error'], values['refunded'], local_order_id),
            )
        else:
            con.execute(
                """INSERT INTO api_order_links(local_order_id,provider_id,remote_order_code,status,delivery_json,error,refunded)
                   VALUES (?,?,?,?,?,?,?)""",
                (local_order_id, provider_id, remote_order_code, status, delivery_json, error, int(refunded or 0)),
            )
        con.commit()


def set_api_remote_stock(local_product_id: int, stock: int):
    with db() as con:
        con.execute(
            "UPDATE api_product_map SET remote_stock=?, last_sync_at=CURRENT_TIMESTAMP WHERE local_product_id=?",
            (max(0, int(stock)), local_product_id),
        )
        con.commit()
...
async def shop_api_request(provider, method: str, endpoint: str, payload: dict | None = None) -> dict:
    ...
    header_name = str(provider['auth_header'] or 'X-Shop-API-Key').strip() or 'X-Shop-API-Key'
    header_value = f"Bearer {key}" if header_name.lower() == 'authorization' else key
        ...
        async with aiohttp.ClientSession(timeout=aiohttp.ClientTimeout(total=20)) as session:
            async with session.request(method.upper(), url, headers=headers, json=payload) as resp:
                ...
                try:
                    ...
                    data = {'raw': raw}
                if resp.status >= 400:
                    msg = data.get('error') or data.get('message') or data.get('detail') or raw or f'HTTP {resp.status}'
                    raise ShopAPIError(str(msg)[:400], resp.status, data)
                if not isinstance(data, dict):
                    raise ShopAPIError('Invalid Shop API response.', resp.status, data)
                return data
    except ShopAPIError:
        raise
    except Exception as exc:
        raise ShopAPIError(str(exc)[:400], None, None) from exc


async def fetch_shop_api_catalog(provider_id: int) -> tuple[dict, list[dict]]:
    """Fetch supplier wallet/profile and current buyable products without modifying local data."""
    provider = get_api_provider(provider_id)
    if not provider or int(provider['active'] or 0) != 1:
        raise ShopAPIError('API provider not found or disabled.')
    if str(provider['provider_type'] or 'generic') != 'shop_v1':
        raise ShopAPIError('This provider is not configured as Shop API v1.')
    me = await shop_api_request(provider, 'GET', provider['balance_endpoint'] or '/me')
    if me.get('ok') is not True:
        raise ShopAPIError('Shop API authentication check failed.')
    data = await shop_api_request(provider, 'GET', provider['products_endpoint'] or '/products')
    products = data.get('products') or []
    if not isinstance(products, list):
        raise ShopAPIError('Shop API /products returned an invalid list.')
    return me, [p for p in products if isinstance(p, dict) and p.get('id') is not None]


def _api_sale_price(cost, profit_percent) -> Decimal:
    cost_d = Decimal(str(cost or '0'))
    profit_d = Decimal(str(profit_percent or '0'))
    if cost_d < 0 or profit_d < 0:
        raise ValueError('Cost and profit percentage cannot be negative.')
    return (cost_d * (Decimal('1') + profit_d / Decimal('100'))).quantize(Decimal('0.001'))


def get_api_product_map_by_remote(provider_id: int, remote_product_id: str):
    with db() as con:
        return con.execute(
            "SELECT * FROM api_product_map WHERE provider_id=? AND remote_product_id=?",
            (provider_id, str(remote_product_id)),
        ).fetchone()


def hide_unselected_api_products() -> None:
    """Hide legacy auto-imported API products without deleting any row/order/history."""
    with db() as con:
        con.execute(
            """UPDATE products SET active=0
               WHERE id IN (
                   SELECT local_product_id FROM api_product_map
                   WHERE COALESCE(selected,0)=0
               )"""
        )
        con.commit()


async def import_selected_shop_product(
    provider_id: int,
    remote_product_id: str,
    profit_percent: Decimal,
    validity: str,
    warranty: str,
) -> tuple[int, bool, dict, Decimal, Decimal]:
    """Import exactly one admin-selected supplier product. Never auto-import the rest."""
    provider = get_api_provider(provider_id)
    if not provider or int(provider['active'] or 0) != 1:
        raise ShopAPIError('API provider not found or disabled.')

    remote_id = str(remote_product_id)
    detail = await shop_api_request(provider, 'GET', f'/products/{remote_id}')
    rp = detail.get('product') or {}
    if not isinstance(rp, dict) or rp.get('id') is None:
        raise ShopAPIError('Product details are unavailable.')

    name = str(rp.get('name') or f'Product {remote_id}').strip()
    description = str(rp.get('description') or '').strip()
    category_obj = rp.get('category') if isinstance(rp.get('category'), dict) else {}
    category_id = str(rp.get('category_id') or category_obj.get('id') or '')
    category = str(category_obj.get('name') or '').strip()
    cost = Decimal(str(rp.get('unit_price') if rp.get('unit_price') is not None else rp.get('list_price') or '0'))
    sell_price = _api_sale_price(cost, profit_percent)
    stock = max(0, int(rp.get('stock_count') or 0))
    min_qty = max(1, int(rp.get('min_qty') or 1))
    max_qty = max(min_qty, int(rp.get('max_qty') or max(min_qty, stock or 1)))
    list_price = str(rp.get('list_price') if rp.get('list_price') is not None else cost)
    image_url = str(rp.get('image_url') or '')

    mapped = get_api_product_map_by_remote(provider_id, remote_id)
    announce_add = mapped is None or int(mapped['selected'] or 0) == 0
    if mapped:
        local_id = int(mapped['local_product_id'])
        with db() as con:
            con.execute(
                """UPDATE products
                   SET name=?, price=?, validity=?, warranty=?, product_type=?, category=?, note=?, active=1
                   WHERE id=?""",
                (name, str(sell_price), validity, warranty, 'Other', category, description, local_id),
            )
            con.execute(
                """UPDATE api_product_map
                   SET remote_category_id=?, remote_stock=?, min_qty=?, max_qty=?, list_price=?, image_url=?,
                       selected=1, cost_price=?, profit_percent=?, last_sync_at=CURRENT_TIMESTAMP
                   WHERE local_product_id=?""",
                (category_id, stock, min_qty, max_qty, list_price, image_url,
                 str(cost), str(profit_percent), local_id),
            )
            con.commit()
    else:
        local_id = create_product(name, sell_price, validity, warranty, 'Other', category, description)
        with db() as con:
            con.execute(
                """INSERT INTO api_product_map(
                       local_product_id,provider_id,remote_product_id,remote_category_id,remote_stock,
                       min_qty,max_qty,list_price,image_url,selected,cost_price,profit_percent
                   ) VALUES (?,?,?,?,?,?,?,?,?,1,?,?)""",
                (local_id, provider_id, remote_id, category_id, stock,
                 min_qty, max_qty, list_price, image_url, str(cost), str(profit_percent)),
            )
            con.commit()
    return local_id, announce_add, rp, cost, sell_price


async def sync_shop_api_provider(provider_id: int) -> dict:
    """Refresh ONLY products the admin selected. No remote product is auto-added."""
    provider = get_api_provider(provider_id)
    if not provider or int(provider['active'] or 0) != 1:
        raise ShopAPIError('API provider not found or disabled.')
    if str(provider['provider_type'] or 'generic') != 'shop_v1':
    ...
    me, remote_products = await fetch_shop_api_catalog(provider_id)
    remote_by_id = {str(p.get('id')): p for p in remote_products}
    with db() as con:
        selected_rows = con.execute(
            "SELECT * FROM api_product_map WHERE provider_id=? AND COALESCE(selected,0)=1",
            (provider_id,),
        ).fetchall()

    updated = unavailable = 0
    for mapped in selected_rows:
        local_id = int(mapped['local_product_id'])
        remote_id = str(mapped['remote_product_id'])
        rp = remote_by_id.get(remote_id)
        if not rp:
            set_api_remote_stock(local_id, 0)
            unavailable += 1
            continue

        cost = Decimal(str(rp.get('unit_price') if rp.get('unit_price') is not None else rp.get('list_price') or '0'))
        profit = Decimal(str(mapped['profit_percent'] or '0'))
        sell_price = _api_sale_price(cost, profit)
        stock = max(0, int(rp.get('stock_count') or 0))
        min_qty = max(1, int(rp.get('min_qty') or 1))
        max_qty = max(min_qty, int(rp.get('max_qty') or max(min_qty, stock or 1)))
        list_price = str(rp.get('list_price') if rp.get('list_price') is not None else cost)
        image_url = str(rp.get('image_url') or '')
        category_id = str(rp.get('category_id') or '')
        with db() as con:
            con.execute("UPDATE products SET price=?, active=1 WHERE id=?", (str(sell_price), local_id))
            con.execute(
                """UPDATE api_product_map
                   SET remote_category_id=?, remote_stock=?, min_qty=?, max_qty=?, list_price=?, image_url=?,
                       cost_price=?, last_sync_at=CURRENT_TIMESTAMP
                   WHERE local_product_id=?""",
                (category_id, stock, min_qty, max_qty, list_price, image_url, str(cost), local_id),
            )
            con.commit()
        updated += 1
    ...
    return {
        ...
        'catalog_count': len(remote_products),
        'balance': me.get('balance'),
        'telegram_id': me.get('telegram_id'),
    }


async def shop_api_background_sync_loop():
    """Keep external Shop API stock/catalog fresh without slowing customer button clicks."""
    await asyncio.sleep(5)
    while True:
        try:
            providers = [
                p for p in list_api_providers()
                if int(p['active'] or 0) == 1 and str(p['provider_type'] or '') == 'shop_v1'
            ]
            for provider in providers:
                try:
                    await sync_shop_api_provider(int(provider['id']))
                except Exception as exc:
                    logging.warning('Background Shop API sync failed for provider %s: %s', provider['id'], exc)
        except asyncio.CancelledError:
            raise
        except Exception:
            logging.exception('Background Shop API sync loop error')
        await asyncio.sleep(SHOP_API_SYNC_SECONDS)
...
async def broadcast_new_product(bot: Bot, pid: int):
    ...
    # New-product announcements are allowed in the connected public channel.
    with db() as con:
        users = con.execute("SELECT telegram_id FROM users WHERE blocked=0").fetchall()
    for row in users:
        try:
...
async def broadcast_stock_added(bot: Bot, pid: int, added: int):
    ...
    # Public channel stays quiet for stock updates.
    with db() as con:
        users = con.execute("SELECT telegram_id FROM users WHERE blocked=0").fetchall()
    for row in users:
        try:
...
async def broadcast_price_update(bot: Bot, pid: int, old_price: Decimal, new_price: Decimal):
    ...
    public_kb = InlineKeyboardMarkup(inline_keyboard=[[InlineKeyboardButton(text="🛒 Buy Now", url=f"https://t.me/{me.username}?start=product_{pid}", style="success")]])
    # Public channel stays quiet for price updates.
    with db() as con:
        users = con.execute("SELECT telegram_id FROM users WHERE blocked=0").fetchall()
    for row in users:
...
async def public_purchase_notice(bot: Bot, order):
    """Public channel policy: no automatic purchase logs.

    The public/connected channel receives only new-product announcements and
    messages the admin explicitly broadcasts.
    """
    return
...
async def _api_refund_once(bot: Bot, order, provider_id: int, reason: str):
    oid = str(order['order_id'])
    link = get_api_order_link(oid)
    if link and int(link['refunded'] or 0):
        return
    amount = Decimal(str(order['total_amount']))
    balance = change_wallet(int(order['telegram_id']), amount, 'API_REFUND', oid, reason[:180])
    save_api_order_link(oid, provider_id, 'REFUNDED', error=reason[:400], refunded=1)
    update_order(oid, status='REFUNDED', paid_at=datetime.now(timezone.utc).isoformat())
    await bot.send_message(
        int(order['telegram_id']),
        "⚠️ <b>Order could not be completed</b>\n\n"
        f"🆔 Order: <code>{html.escape(oid)}</code>\n"
        f"💰 Returned to wallet: <b>{money(amount)} {INVOICE_CURRENCY}</b>\n"
        f"👛 Balance: <b>{money(balance)} {INVOICE_CURRENCY}</b>\n\n"
        "No balance was lost. You can try again or choose another product.",
        reply_markup=back_home(int(order['telegram_id'])),
    )


async def complete_shop_api_order(bot: Bot, order, method: str, mapped) -> None:
    oid = str(order['order_id'])
    uid = int(order['telegram_id'])
    provider_id = int(mapped['provider_id'])
    provider = get_api_provider(provider_id)
    if not provider:
        await _api_refund_once(bot, order, provider_id, 'Product service configuration is missing.')
        return

    previous = get_api_order_link(oid)
    if previous:
        prev_status = str(previous['status'] or '').upper()
        if prev_status == 'FULFILLED' and previous['delivery_json']:
            try:
                items = json.loads(previous['delivery_json']) or []
            except Exception:
                items = []
        elif prev_status in {'SUBMITTING', 'REVIEW'}:
            await bot.send_message(
                uid,
                "🕒 <b>Payment received</b>\n\n"
                f"Order <code>{html.escape(oid)}</code> is waiting for processing confirmation. "
                "It will not be submitted twice automatically.",
            )
            return
        elif prev_status == 'REFUNDED':
            return
        else:
            items = None
    else:
    ...
    if items is None:
        # Mark before the network call. If the connection dies after supplier fulfillment,
        # we stop automatic retries to prevent accidental duplicate purchases.
        save_api_order_link(oid, provider_id, 'SUBMITTING')
        try:
            remote_id_raw = str(mapped['remote_product_id'])
            remote_id = int(remote_id_raw) if remote_id_raw.isdigit() else remote_id_raw
            response = await shop_api_request(
                provider,
                'POST',
                provider['order_endpoint'] or '/orders',
                {'product_id': remote_id, 'quantity': int(order['quantity'])},
            )
            if response.get('ok') is not True:
                raise ShopAPIError(str(response.get('error') or 'Order was not accepted.'), 400, response)
            items = response.get('delivered_keys') or []
            if not isinstance(items, list):
                items = [str(items)]
            remote_code = str(response.get('order_code') or '')
            save_api_order_link(oid, provider_id, 'FULFILLED', remote_order_code=remote_code, delivery=items)
            set_api_remote_stock(int(order['product_id']), max(0, int(mapped['remote_stock'] or 0) - int(order['quantity'])))
        except ShopAPIError as exc:
            # 4xx is a definite rejection according to the supplied Shop API docs.
            # Refund locally. Network/5xx is ambiguous, so do not retry/refund automatically.
            if exc.status in {400, 401, 404, 409}:
                await _api_refund_once(bot, order, provider_id, str(exc))
                await safe_send(
                    bot, ADMIN_ALERT_CHANNEL_ID,
                    f"⚠️ <b>SHOP API ORDER REFUNDED</b>\n\n🆔 <code>{html.escape(oid)}</code>\n"
                    f"📦 {html.escape(str(order['product_name']))}\n❌ {html.escape(str(exc))}",
                )
                return
            save_api_order_link(oid, provider_id, 'REVIEW', error=str(exc)[:400])
            update_order(oid, payment_method=method, status='PAID_API_REVIEW', paid_at=datetime.now(timezone.utc).isoformat())
            await bot.send_message(
                uid,
                "🕒 <b>Payment received — confirmation pending</b>\n\n"
                f"🆔 Order: <code>{html.escape(oid)}</code>\n"
                "The product service returned an uncertain result, so the bot will NOT retry automatically. "
                "This protects you from a duplicate order. Admin has been notified.",
            )
            await safe_send(
                bot, ADMIN_ALERT_CHANNEL_ID,
                f"🚨 <b>SHOP API REVIEW NEEDED</b>\n\n🆔 <code>{html.escape(oid)}</code>\n"
                f"📦 {html.escape(str(order['product_name']))}\n⚠️ {html.escape(str(exc))}",
            )
            return

    delivered = "\n".join(f"{i}. <code>{html.escape(str(x))}</code>" for i, x in enumerate(items, 1))
    if not delivered:
        delivered = "<i>Order completed. No text delivery payload was returned.</i>"
    update_order(
        oid, payment_method=method, status='COMPLETED', delivered_content=delivered,
        paid_at=datetime.now(timezone.utc).isoformat(),
    )
    product = get_product(int(order['product_id']))
    copy_raw = "\n".join(str(x) for x in items) if items else str(get_api_order_link(oid)['remote_order_code'] or oid)
    ...
    await bot.send_message(
        ...
        "📦 Your product has been delivered automatically.\n\n"
        f"🆔 Order: <code>{html.escape(oid)}</code>\n"
        f"📦 Product: <b>{html.escape(str(order['product_name']))}</b>\n"
    ...
    if is_email_verified(uid):
        ...
        await bot.send_message(
            ...
            "You can set and verify an email now or later from Settings.",
...
async def complete_paid_order(bot: Bot, oid: str, method: str):
    order = get_order(oid)
    if not order or str(order["status"] or "").startswith("REFUNDED") or order["status"] == "COMPLETED":
        return
    mapped = get_api_product_map(int(order["product_id"]))
    if mapped:
        await complete_shop_api_order(bot, order, method, mapped)
        return
    ...
    if is_email_verified(uid):
        ...
        if sent:
            ...
            await bot.send_message(
                ...
                "Admin: check Brevo Email API settings if this keeps happening.",
            )
    else:
...
async def purchase_confirm(callback: CallbackQuery):
    ...
    if stock_count(pid) < qty:
        return await callback.answer(tr(callback.from_user.id,"not_enough"), show_alert=True)
    mapped = get_api_product_map(pid)
    if mapped:
        min_qty = max(1, int(mapped["min_qty"] or 1))
        max_qty = max(min_qty, int(mapped["max_qty"] or min_qty))
        if qty < min_qty or qty > min(max_qty, stock_count(pid)):
            return await callback.answer(
                f"Quantity must be between {min_qty} and {min(max_qty, stock_count(pid))}.",
                show_alert=True,
            )
...
async def pay_direct(callback: CallbackQuery, state: FSMContext):
    ...
    # Show one clean status only; replace it with the invoice as soon as PayHub responds.
    await edit(
        callback,
...
async def email_received(message: Message, state: FSMContext, bot: Bot):
    ...
    if not sent:
        ...
        return await bot.send_message(
            ...
            "Check Admin → Bot Settings → Brevo Email API, then try Set/Change Email again.",
...
def bot_settings_kb():
    return InlineKeyboardMarkup(inline_keyboard=[
        ...
        [
            ...
            InlineKeyboardButton(text="📧 Brevo Email API", callback_data="admin:cfg:brevo", style="primary"),
        ],
        [
...
async def admin_bot_settings(callback: CallbackQuery, state: FSMContext):
    ...
    await edit(
        callback,
        "⚙️ <b>Bot Settings</b>\n\n"
        "Configure channels, referral, Brevo Email API, AI Chat and Payment API from Admin Panel.\n"
...
async def admin_cfg_choose(callback: CallbackQuery, state: FSMContext):
    ...
    prompt = {
        ...
        "brevo":
            "📧 <b>Brevo Email API</b>\n\n"
            "Send exactly 3 lines:\n"
            "1) Brevo API Key  (xkeysib-...)\n"
            "2) Verified Sender Email\n"
            "3) Sender Name    (example: Premium Hub Store)\n\n"
            "The sender email must be verified in Brevo. This uses HTTPS, not SMTP.",
        "smtp":
            "📧 <b>Legacy SMTP / Invoice Email</b>\n\n"
...
async def admin_cfg_received(message: Message, state: FSMContext):
    ...
    try:
        if section == "public_channel":
            ...
            if len(lines) != 3:
                raise ValueError("Please send exactly 3 lines: API Key, Sender Email, Sender Name.")
            api_key = lines[0].strip()
            sender_email = lines[1].strip().lower()
            sender_name = lines[2].strip() or BOT_NAME
            if not api_key:
                raise ValueError("Brevo API Key cannot be empty.")
            if not EMAIL_RE.fullmatch(sender_email):
                raise ValueError("Send a valid verified sender email.")
            set_app_setting("brevo_api_key", api_key)
            set_app_setting("brevo_sender_email", sender_email)
            set_app_setting("brevo_sender_name", sender_name)
            set_app_setting("brevo_api_url", "https://api.brevo.com/v3/smtp/email")
...
async def admin_api_manager(callback: CallbackQuery, state: FSMContext):
    ...
    lines = [
        "🔌 <b>API Manager</b>", "",
        "Shop API products are <b>manual-add only</b>. Connecting an API never adds its whole catalog.",
    ]
    for p in providers:
        ptype = str(p['provider_type'] or 'generic')
        badge = '🛒 Shop API v1' if ptype == 'shop_v1' else '🔌 Generic'
        lines.append(f"• #{p['id']} <b>{html.escape(p['name'])}</b> — {badge} — {'Active' if p['active'] else 'Disabled'}")
    kb = InlineKeyboardMarkup(inline_keyboard=[
        [
            InlineKeyboardButton(text="➕ Add API", callback_data="admin:api_add_menu", style="success"),
            InlineKeyboardButton(text="🗑 Remove API", callback_data="admin:api_remove_menu", style="danger"),
        ],
        [InlineKeyboardButton(text="➕ Add API Product", callback_data="admin:shop_api_choose_provider", style="success")],
        [InlineKeyboardButton(text="💰 Supplier Balance", callback_data="admin:shop_api_balance", style="primary")],
        [InlineKeyboardButton(text="🔄 Refresh Selected Stock/Price", callback_data="admin:shop_api_sync", style="primary")],
        [InlineKeyboardButton(text="◀ Back", callback_data="admin:home", style="danger")],
    ])
    await edit(callback, "\n".join(lines), kb)


@router.callback_query(F.data == "admin:api_add_menu")
async def admin_api_add_menu(callback: CallbackQuery, state: FSMContext):
    if not is_admin(callback.from_user.id):
        return
    await state.clear()
    kb = InlineKeyboardMarkup(inline_keyboard=[
        [InlineKeyboardButton(text="🛒 Shop API v1", callback_data="admin:shop_api_connect", style="success")],
        [InlineKeyboardButton(text="🔌 Generic API Provider", callback_data="admin:api_add", style="primary")],
        [InlineKeyboardButton(text="◀ Back", callback_data="admin:api_manager", style="danger")],
    ])
    await edit(
        callback,
        "➕ <b>Add API</b>\n\nChoose the API type you want to connect.\n\n"
        "🛒 <b>Shop API v1</b> — catalog, wallet and instant order delivery.\n"
        "🔌 <b>Generic API</b> — custom provider/endpoints.",
        kb,
    )


@router.callback_query(F.data == "admin:api_remove_menu")
async def admin_api_remove_menu(callback: CallbackQuery, state: FSMContext):
    if not is_admin(callback.from_user.id):
        return
    await state.clear()
    providers = [p for p in list_api_providers() if int(p["active"] or 0) == 1]
    if not providers:
        return await callback.answer("No active API provider to remove.", show_alert=True)

    rows = []
    lines = [
        "🗑 <b>Remove API Provider</b>",
        "",
        "Choose an API to disconnect.",
        "",
        "🛡 Removing an API will NOT delete users, balances, orders or purchase history.",
        "Its API products will be hidden and the stored API key will be erased.",
        "",
    ]
    for p in providers:
        ptype = str(p["provider_type"] or "generic")
        badge = "🛒" if ptype == "shop_v1" else "🔌"
        lines.append(f"{badge} #{p['id']} <b>{html.escape(str(p['name']))}</b>")
        rows.append([InlineKeyboardButton(
            text=f"🗑 {p['name']} (#{p['id']})",
            callback_data=f"admin:api_remove_pick:{p['id']}",
            style="danger",
        )])
    rows.append([InlineKeyboardButton(text="◀ Back", callback_data="admin:api_manager", style="primary")])
    await edit(callback, "\n".join(lines), InlineKeyboardMarkup(inline_keyboard=rows))


@router.callback_query(F.data.startswith("admin:api_remove_pick:"))
async def admin_api_remove_pick(callback: CallbackQuery):
    if not is_admin(callback.from_user.id):
        return
    try:
        provider_id = int(callback.data.rsplit(":", 1)[1])
    except Exception:
        return await callback.answer("Invalid provider.", show_alert=True)
    p = get_api_provider(provider_id)
    if not p or int(p["active"] or 0) != 1:
        return await callback.answer("Provider not found or already removed.", show_alert=True)

    ptype = "Shop API v1" if str(p["provider_type"] or "generic") == "shop_v1" else "Generic API"
    with db() as con:
        mapped_count = int(con.execute(
            "SELECT COUNT(*) AS c FROM api_product_map WHERE provider_id=?",
            (provider_id,),
        ).fetchone()["c"] or 0)
    kb = InlineKeyboardMarkup(inline_keyboard=[
        [InlineKeyboardButton(
            text="✅ Yes, Remove API",
            callback_data=f"admin:api_remove_do:{provider_id}",
            style="danger",
        )],
        [InlineKeyboardButton(text="❌ Cancel", callback_data="admin:api_remove_menu", style="primary")],
    ])
    await edit(
        callback,
        "⚠️ <b>Confirm API Removal</b>\n\n"
        f"Provider: <b>{html.escape(str(p['name']))}</b>\n"
        f"Type: <b>{ptype}</b>\n"
        f"Linked API products: <b>{mapped_count}</b>\n\n"
        "After removal:\n"
        "• API connection is disabled\n"
        "• Stored API key is erased\n"
        "• Linked API products are hidden\n"
        "• Background sync/order calls stop\n"
        "• Users, balances, previous orders and history stay safe\n\n"
        "Continue?",
        kb,
    )


@router.callback_query(F.data.startswith("admin:api_remove_do:"))
async def admin_api_remove_do(callback: CallbackQuery, state: FSMContext):
    if not is_admin(callback.from_user.id):
        return
    try:
        provider_id = int(callback.data.rsplit(":", 1)[1])
        result = safely_remove_api_provider(provider_id)
    except Exception as exc:
        logging.exception("API provider removal failed")
        return await callback.answer(str(exc)[:180], show_alert=True)
    await state.clear()
    await edit(
        callback,
        "✅ <b>API Removed Safely</b>\n\n"
        f"Provider: <b>{html.escape(result['name'])}</b>\n"
        f"Hidden linked products: <b>{result['hidden_products']}</b>\n"
        "🔐 Stored API key erased.\n\n"
        "🛡 No customer balance, user data, previous order or purchase history was deleted.\n"
        "You can add/connect another API anytime.",
        InlineKeyboardMarkup(inline_keyboard=[
            [InlineKeyboardButton(text="➕ Add API", callback_data="admin:api_add_menu", style="success")],
            [InlineKeyboardButton(text="◀ API Manager", callback_data="admin:api_manager", style="primary")],
        ]),
...
@router.callback_query(F.data == "admin:shop_api_connect")
async def admin_shop_api_connect(callback: CallbackQuery, state: FSMContext):
    if not is_admin(callback.from_user.id): return
    await state.set_state(AdminShopAPIState.config)
    await edit(
        callback,
        "🛒 <b>Connect Shop API v1</b>\n\n"
        "Send exactly 3 lines:\n"
        "1) Provider Name\n"
        "2) Base URL\n"
        "3) Shop API Key\n\n"
        "Example Base URL:\n"
        "<code>https://shopbot.00969600.xyz/shop-api/v1</code>\n\n"
        "✅ Connecting only saves the API. <b>No product is auto-added.</b>\n"
        "🔐 The key is private and never shown to customers.",
        InlineKeyboardMarkup(inline_keyboard=[[InlineKeyboardButton(text='◀ Cancel', callback_data='admin:api_manager', style='danger')]]),
    )


@router.message(AdminShopAPIState.config)
async def admin_shop_api_config(message: Message, state: FSMContext):
    if not is_admin(message.from_user.id): return
    lines = [x.strip() for x in (message.text or '').splitlines() if x.strip()]
    if len(lines) != 3:
        return await message.answer('❌ Send exactly 3 lines: Name, Base URL, API Key.')
    name, base_url, key = lines
    if not base_url.startswith(('http://', 'https://')):
        return await message.answer('❌ Base URL must start with http:// or https://')
    if not key.startswith('sk_shop_'):
        return await message.answer('❌ Shop API key should start with sk_shop_')
    temp = {'base_url': base_url.rstrip('/'), 'api_key': key, 'auth_header': 'X-Shop-API-Key'}
    try:
        me = await shop_api_request(temp, 'GET', '/me')
        if me.get('ok') is not True:
            raise ShopAPIError('Authentication failed.')
    except Exception as exc:
        logging.exception('Shop API connection test failed')
        return await message.answer(f"❌ Connection failed: {html.escape(str(exc))}")
    pid = add_shop_api_provider(name, base_url, key)
    with suppress(Exception):
        await message.delete()
    await state.clear()
    await message.answer(
        "✅ <b>Shop API Connected</b>\n\n"
        f"Provider ID: <code>{pid}</code>\n"
        f"Supplier Balance: <b>{money(me.get('balance')) if me.get('balance') is not None else '-'}</b>\n\n"
        "📦 <b>No products were imported.</b>\n"
        "Go to <b>API Manager → Add API Product</b>, choose exactly the product you want, "
        "then set your profit percentage, validity and warranty.\n\n"
        "🛡 Existing users, balances, products, orders and stock were not changed.",
        reply_markup=admin_kb(),
    )


@router.callback_query(F.data == "admin:shop_api_choose_provider")
async def admin_shop_api_choose_provider(callback: CallbackQuery):
    if not is_admin(callback.from_user.id): return
    providers = [p for p in list_api_providers() if int(p['active'] or 0) == 1 and str(p['provider_type'] or '') == 'shop_v1']
    if not providers:
        return await callback.answer('No Shop API v1 provider connected.', show_alert=True)
    rows = [[InlineKeyboardButton(text=f"🛒 {p['name']}", callback_data=f"admin:shop_api_products:{p['id']}:1", style='success')] for p in providers]
    rows.append([InlineKeyboardButton(text='◀ Back', callback_data='admin:api_manager', style='danger')])
    await edit(callback, "➕ <b>Add API Product</b>\n\nChoose a supplier. Only the product you select will be added.", InlineKeyboardMarkup(inline_keyboard=rows))


async def show_shop_api_products(callback: CallbackQuery, provider_id: int, page: int = 1):
    provider = get_api_provider(provider_id)
    if not provider:
        return await callback.answer('Provider not found.', show_alert=True)
    try:
        me, products = await fetch_shop_api_catalog(provider_id)
    except Exception as exc:
        return await edit(callback, f"❌ <b>Could not load products</b>\n\n{html.escape(str(exc))}", InlineKeyboardMarkup(inline_keyboard=[[InlineKeyboardButton(text='◀ Back', callback_data='admin:api_manager', style='danger')]]))
    per_page = 8
    pages = max(1, ceil(len(products) / per_page))
    page = max(1, min(page, pages))
    chunk = products[(page-1)*per_page: page*per_page]
    rows = []
    for rp in chunk:
        rid = str(rp.get('id'))
        name = str(rp.get('name') or f'Product {rid}')
        cost = money(rp.get('unit_price') if rp.get('unit_price') is not None else rp.get('list_price') or '0')
        stock = int(rp.get('stock_count') or 0)
        mapped = get_api_product_map_by_remote(provider_id, rid)
        selected = bool(mapped and int(mapped['selected'] or 0) == 1)
        prefix = '✅' if selected else '➕'
        label = f"{prefix} {name} • ${cost} • S:{stock}"
        if len(label) > 64:
            label = label[:63] + '…'
        rows.append([InlineKeyboardButton(text=label, callback_data=f"admin:shop_api_pick:{provider_id}:{rid}", style='success' if not selected else 'primary')])
    nav=[]
    if page>1:
        nav.append(InlineKeyboardButton(text='⬅ Prev', callback_data=f'admin:shop_api_products:{provider_id}:{page-1}', style='primary'))
    nav.append(InlineKeyboardButton(text=f'📊 {page}/{pages}', callback_data='noop', style='primary'))
    if page<pages:
        nav.append(InlineKeyboardButton(text='Next ➡', callback_data=f'admin:shop_api_products:{provider_id}:{page+1}', style='primary'))
    rows.append(nav)
    rows.append([InlineKeyboardButton(text='◀ Back', callback_data='admin:shop_api_choose_provider', style='danger')])
    balance = money(me.get('balance')) if me.get('balance') is not None else '-'
    await edit(
        callback,
        f"🛒 <b>{html.escape(provider['name'])} Products</b>\n\n"
        f"💰 Supplier Balance: <b>{balance}</b>\n"
        "👇 Choose only the product you want to add.\n"
        "✅ = already added to your shop.",
        InlineKeyboardMarkup(inline_keyboard=rows),
    )


@router.callback_query(F.data.startswith('admin:shop_api_products:'))
async def admin_shop_api_products(callback: CallbackQuery):
    if not is_admin(callback.from_user.id): return
    _, _, provider_raw, page_raw = callback.data.split(':', 3)
    await show_shop_api_products(callback, int(provider_raw), int(page_raw))


@router.callback_query(F.data.startswith('admin:shop_api_pick:'))
async def admin_shop_api_pick(callback: CallbackQuery, state: FSMContext):
    if not is_admin(callback.from_user.id): return
    _, _, provider_raw, remote_id = callback.data.split(':', 3)
    ...
    try:
        ...
        rp = detail.get('product') or {}
        if not isinstance(rp, dict) or rp.get('id') is None:
            raise ShopAPIError('Product details unavailable.')
    except Exception as exc:
        return await callback.answer(str(exc)[:180], show_alert=True)
    cost = Decimal(str(rp.get('unit_price') if rp.get('unit_price') is not None else rp.get('list_price') or '0'))
    await state.set_state(AdminShopAPIState.product_setup)
    await state.update_data(shop_provider_id=provider_id, shop_remote_product_id=str(remote_id))
    await edit(
        callback,
        "➕ <b>Add This API Product</b>\n\n"
        f"📦 Product: <b>{html.escape(str(rp.get('name') or remote_id))}</b>\n"
        f"💵 Supplier Cost: <b>{money(cost)} {INVOICE_CURRENCY}</b>\n"
        f"📊 Supplier Stock: <b>{int(rp.get('stock_count') or 0)}</b>\n\n"
        "Send exactly <b>3 lines</b>:\n"
        "1) Profit %  (example: 20)\n"
        "2) Validity  (example: 1 Month / N/A)\n"
        "3) Warranty  (example: 30 Days / No Warranty)\n\n"
        "Example:\n<code>20\n1 Month\n30 Days</code>",
        InlineKeyboardMarkup(inline_keyboard=[[InlineKeyboardButton(text='◀ Cancel', callback_data='admin:api_manager', style='danger')]]),
    )


@router.message(AdminShopAPIState.product_setup)
async def admin_shop_api_product_setup(message: Message, state: FSMContext):
    if not is_admin(message.from_user.id): return
    lines = [x.strip() for x in (message.text or '').splitlines() if x.strip()]
    if len(lines) != 3:
        return await message.answer('❌ Send exactly 3 lines: Profit %, Validity, Warranty.')
    try:
        profit = Decimal(lines[0])
        if profit < 0 or profit > Decimal('10000'):
            raise ValueError
    except Exception:
        return await message.answer('❌ Invalid profit %. Example: 20')
    data = await state.get_data()
    provider_id = int(data.get('shop_provider_id') or 0)
    remote_id = str(data.get('shop_remote_product_id') or '')
    provider = get_api_provider(provider_id)
    if not provider or not remote_id:
        await state.clear()
        return await message.answer('❌ Session expired. Open API Manager and try again.', reply_markup=admin_kb())
    try:
        detail = await shop_api_request(provider, 'GET', f'/products/{remote_id}')
        rp = detail.get('product') or {}
        cost = Decimal(str(rp.get('unit_price') if rp.get('unit_price') is not None else rp.get('list_price') or '0'))
        sell = _api_sale_price(cost, profit)
    except Exception as exc:
        return await message.answer(f"❌ Could not refresh product: {html.escape(str(exc))}")
    await state.update_data(shop_profit=str(profit), shop_validity=lines[1], shop_warranty=lines[2])
    existing = get_api_product_map_by_remote(provider_id, remote_id)
    action = 'Update Existing Product' if existing else 'Add Product'
    await message.answer(
        "🧾 <b>Confirm API Product</b>\n\n"
        f"📦 Product: <b>{html.escape(str(rp.get('name') or remote_id))}</b>\n"
        f"💵 Supplier Cost: <b>{money(cost)} {INVOICE_CURRENCY}</b>\n"
        f"📈 Profit: <b>{money(profit)}%</b>\n"
        f"💰 Your Selling Price: <b>{money(sell)} {INVOICE_CURRENCY}</b>\n"
        f"📊 Live Stock: <b>{int(rp.get('stock_count') or 0)}</b>\n"
        f"📅 Validity: <b>{html.escape(lines[1])}</b>\n"
        f"🛡 Warranty: <b>{html.escape(lines[2])}</b>\n\n"
        "Only this product will be added. Other supplier products stay hidden.",
        reply_markup=InlineKeyboardMarkup(inline_keyboard=[
            [InlineKeyboardButton(text=f'✅ {action}', callback_data='admin:shop_api_import_confirm', style='success')],
            [InlineKeyboardButton(text='❌ Cancel', callback_data='admin:api_manager', style='danger')],
        ]),
    )


@router.callback_query(F.data == 'admin:shop_api_import_confirm')
async def admin_shop_api_import_confirm(callback: CallbackQuery, state: FSMContext):
    if not is_admin(callback.from_user.id): return
    data = await state.get_data()
    try:
        provider_id = int(data['shop_provider_id'])
        remote_id = str(data['shop_remote_product_id'])
        profit = Decimal(str(data['shop_profit']))
        validity = str(data['shop_validity'])
        warranty = str(data['shop_warranty'])
        local_id, announce_add, rp, cost, sell = await import_selected_shop_product(
            provider_id, remote_id, profit, validity, warranty
        )
    except Exception as exc:
        logging.exception('Manual Shop API product import failed')
        return await callback.answer(str(exc)[:180], show_alert=True)
    await state.clear()
    await edit(
        callback,
        "✅ <b>API Product Added</b>\n\n"
        f"🆔 Local Product ID: <code>{local_id}</code>\n"
        f"📦 Product: <b>{html.escape(str(rp.get('name') or remote_id))}</b>\n"
        f"💵 Supplier Cost: <b>{money(cost)} {INVOICE_CURRENCY}</b>\n"
        f"📈 Profit: <b>{money(profit)}%</b>\n"
        f"💰 Selling Price: <b>{money(sell)} {INVOICE_CURRENCY}</b>\n"
        f"📊 Stock: <b>{int(rp.get('stock_count') or 0)}</b>\n\n"
        "👤 Customers will see it exactly like a normal shop product.\n"
        "🔄 Supplier stock and cost-based selling price will keep refreshing automatically.",
        InlineKeyboardMarkup(inline_keyboard=[
            [InlineKeyboardButton(text='➕ Add Another API Product', callback_data='admin:shop_api_choose_provider', style='success')],
            [InlineKeyboardButton(text='◀ API Manager', callback_data='admin:api_manager', style='primary')],
        ]),
    )
    if announce_add:
        with suppress(Exception):
            await broadcast_new_product(callback.bot, local_id)


@router.callback_query(F.data == 'admin:shop_api_balance')
async def admin_shop_api_balance(callback: CallbackQuery):
    if not is_admin(callback.from_user.id): return
    providers = [p for p in list_api_providers() if int(p['active'] or 0) == 1 and str(p['provider_type'] or '') == 'shop_v1']
    if not providers:
        return await callback.answer('No Shop API v1 provider connected.', show_alert=True)
    lines = ["💰 <b>Supplier API Balance</b>", ""]
    for p in providers:
        try:
            me = await shop_api_request(p, 'GET', p['balance_endpoint'] or '/me')
            lines.append(f"• <b>{html.escape(p['name'])}</b>: {money(me.get('balance')) if me.get('balance') is not None else '-'}")
        except Exception as exc:
            lines.append(f"• <b>{html.escape(p['name'])}</b>: ❌ {html.escape(str(exc))}")
    lines += ["", "ℹ️ This Shop API does not provide wallet top-up. Increase supplier balance from the supplier's Telegram shop/account, then tap this button again."]
    await edit(callback, "\n".join(lines), InlineKeyboardMarkup(inline_keyboard=[[InlineKeyboardButton(text='◀ Back', callback_data='admin:api_manager', style='danger')]]))


@router.callback_query(F.data == "admin:shop_api_sync")
async def admin_shop_api_sync(callback: CallbackQuery):
    if not is_admin(callback.from_user.id): return
    providers = [p for p in list_api_providers() if int(p['active'] or 0) == 1 and str(p['provider_type'] or '') == 'shop_v1']
    if not providers:
        return await callback.answer('No Shop API v1 provider connected.', show_alert=True)
    await edit(callback, '🔄 <b>Refreshing selected API products...</b>', InlineKeyboardMarkup(inline_keyboard=[[InlineKeyboardButton(text='◀ Back', callback_data='admin:api_manager', style='danger')]]))
    summaries = []
    for p in providers:
        try:
            r = await sync_shop_api_provider(int(p['id']))
            summaries.append(
                f"✅ <b>{html.escape(p['name'])}</b> — Selected: {r['selected']} | Updated: {r['updated']} | Out of stock/unavailable: {r['unavailable']} | Supplier catalog: {r['catalog_count']} | Balance: {money(r['balance']) if r['balance'] is not None else '-'}"
            )
        except Exception as exc:
            summaries.append(f"❌ <b>{html.escape(p['name'])}</b> — {html.escape(str(exc))}")
    await edit(
        callback,
        "🔄 <b>Selected API Products Refreshed</b>\n\n" + "\n".join(summaries) +
        "\n\n✅ No unselected supplier product was added.\n🛡 Existing users, balances, orders and local stock are preserved.",
        InlineKeyboardMarkup(inline_keyboard=[[InlineKeyboardButton(text='◀ Back to API Manager', callback_data='admin:api_manager', style='primary')]]),
    )
...
async def main():
    ...
    init_db()
    migrate_v23_schema()
    # V25.7 keeps manual Shop API products and adds safe Add/Remove API management.
    # Legacy auto-imported mappings are hidden, never deleted, so all orders/history remain safe.
    hide_unselected_api_products()
    refresh_runtime_settings()
    ...
    shop_sync_task = asyncio.create_task(shop_api_background_sync_loop())
