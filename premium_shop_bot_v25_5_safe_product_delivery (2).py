# Premium Hub Store Bot - V25.5 Safe Product Delivery
# Python 3.10+
# Dependencies:
...
COPY_TEXT_MAX_UNITS = 256
DELIVERY_CODE_ESCAPED_LIMIT = 3000
DELIVERY_RAW_CHAR_LIMIT = 1800


def _telegram_text_units(value: str) -> int:
    """Telegram limits are safest to measure as UTF-16 code units."""
    return len(str(value or "").encode("utf-16-le")) // 2


def _decode_reserved_delivery_items(raw_value) -> list[str]:
    """Read product items stored by the safe-delivery flow.

    New orders store reserved stock as JSON. Older V25.4 builds stored an HTML
    string such as: 1. <code>...</code>. Supporting both lets admin redeliver
    purchases that were charged before this fix but failed at Telegram send.
    """
    ...
    # Legacy V25.4 delivery format.
    matches = re.findall(r"<code>(.*?)</code>", raw, flags=re.S)
    if matches:
        return [html.unescape(x) for x in matches]

    return [html.unescape(raw)]


def _split_product_for_code(
    raw: str,
    max_escaped: int = DELIVERY_CODE_ESCAPED_LIMIT,
    max_raw_chars: int = DELIVERY_RAW_CHAR_LIMIT,
) -> list[str]:
    """Split arbitrary stock text so every Telegram HTML message stays safe."""
    raw = str(raw or "")
    if not raw:
        return [""]
    ...
    for ch in raw:
        esc_len = len(html.escape(ch))
        if current and (
            escaped_len + esc_len > max_escaped
            or len(current) >= max_raw_chars
        ):
...
def _copy_text_allowed(value: str) -> bool:
    value = str(value or "").strip()
    units = _telegram_text_units(value)
    return bool(value) and 1 <= units <= COPY_TEXT_MAX_UNITS


def _delivery_actions_kb(oid: str, items: list[str]) -> InlineKeyboardMarkup:
    """Build a Telegram-safe delivery keyboard.

    The Copy button is optional. If product data is too long for Telegram's
    CopyTextButton, it is omitted instead of breaking the whole delivery.
    """
    copy_raw = "\n".join(str(x) for x in items).strip()
    rows = []

    if _copy_text_allowed(copy_raw):
        rows.append([
            InlineKeyboardButton(
                text="📋 Copy Product Details",
                copy_text=CopyTextButton(text=copy_raw),
                style="primary",
            )
        ])
    ...
    rows.extend([
        [InlineKeyboardButton(
            ...
            callback_data=f"invoice:download:{oid}",
            style="success",
        )],
        [InlineKeyboardButton(
            text="🛒 Go to Shop",
...
async def _send_reserved_product_items(bot: Bot, uid: int, items: list[str]):
    """Deliver every reserved stock item without depending on CopyTextButton."""
    total_items = len(items)

    for item_index, item in enumerate(items, 1):
        parts = _split_product_for_code(str(item))
        total_parts = len(parts)

        for part_index, part in enumerate(parts, 1):
            if total_items == 1 and total_parts == 1:
                title = "🔐 <b>YOUR PRODUCT</b>"
            elif total_parts == 1:
                title = f"🔐 <b>YOUR PRODUCT {item_index}/{total_items}</b>"
            else:
                title = (
                    f"🔐 <b>YOUR PRODUCT {item_index}/{total_items}</b> "
...
async def _send_delivery_action_message(
    bot: Bot,
    uid: int,
    oid: str,
    items: list[str],
):
    """Send invoice/shop controls; an invalid copy button can never block delivery."""
    copy_raw = "\n".join(str(x) for x in items).strip()
    copy_available = _copy_text_allowed(copy_raw)

    text = (
        "✅ <b>Product Delivery Complete</b>\n\n"
        "⚠️ Please save your product details securely."
    )

    if not copy_available:
        text += (
            "\n\n📋 The product is too long for Telegram's Copy button, "
            "so tap/hold the product text above to copy it."
        )

    kb = _delivery_actions_kb(oid, items)
    ...
    try:
        ...
        # Final safety net if Telegram rejects CopyTextButton for any reason.
        if "BUTTON_COPY_TEXT_INVALID" not in str(exc).upper():
            raise

        logging.warning(
            "Telegram rejected CopyTextButton for order=%s; retrying without it",
            oid,
        )
...
async def complete_paid_order(bot: Bot, oid: str, method: str):
    ...
    # If a previous delivery attempt already reserved stock, reuse those exact
    # items. Never consume another stock row for the same paid order.
    items = []
    if order["status"] == "DELIVERY_PENDING" and order["delivered_content"]:
        items = _decode_reserved_delivery_items(order["delivered_content"])
    ...
    if not items:
        ...
        if not items:
            ...
            await bot.send_message(
                ...
                f"Order <code>{html.escape(oid)}</code> is waiting for stock. "
                "Delivery will be automatic when stock is added.",
            )
            return

        # Save the exact reserved product before making Telegram send calls.
        # If Telegram/network fails, this order can safely retry later.
        update_order(
            oid,
            payment_method=method,
            status="DELIVERY_PENDING",
            delivered_content=json.dumps(items, ensure_ascii=False),
            paid_at=datetime.now(timezone.utc).isoformat(),
        )
        order = get_order(oid)
    ...
    try:
        # Summary has no risky CopyTextButton and no large product payload.
        await bot.send_message(
            uid,
            "✅ <b>Purchase Successful!</b>\n\n"
            "📦 Your payment is confirmed and your product is being delivered below.\n\n"
        ...
        # Product details are sent independently and safely chunked.
        await _send_reserved_product_items(bot, uid, items)

    except Exception:
        logging.exception(
            "Product delivery failed after payment; order=%s remains DELIVERY_PENDING",
            oid,
        )
        with suppress(Exception):
            await bot.send_message(
                uid,
                "⚠️ <b>Payment received — delivery is pending</b>\n\n"
                f"Order: <code>{html.escape(oid)}</code>\n"
                "You do not need to pay again. Please contact support if delivery "
                "does not arrive after retry.",
                reply_markup=payhub_fail_kb(),
            )
        return

    # Mark complete ONLY after the actual product text was accepted by Telegram.
    update_order(
        oid,
        payment_method=method,
        status="COMPLETED",
        delivered_content=json.dumps(items, ensure_ascii=False),
        paid_at=order["paid_at"] or datetime.now(timezone.utc).isoformat(),
    )

    # Optional action/copy controls can never block the actual delivery.
    with suppress(Exception):
        await _send_delivery_action_message(bot, uid, oid, items)
    ...
    if is_email_verified(uid):
        ...
        email_prompt_kb = InlineKeyboardMarkup(inline_keyboard=[
            [InlineKeyboardButton(
                text="📧 Set Email",
                callback_data=f"email:after:{oid}",
                style="success",
            )],
            [InlineKeyboardButton(
                text="⏭ Skip",
                callback_data=f"email:skip_after:{oid}",
                style="primary",
            )],
        ])
        await bot.send_message(
...
async def process_waiting_orders(bot: Bot, pid: int):
    while True:
        ...
        await complete_paid_order(
            bot,
            row["order_id"],
            row["payment_method"] or "PAID",
        )


async def bot_send_redelivery(
    bot: Bot,
    uid: int,
    oid: str,
    order,
...
async def admin_redeliver_order(message: Message):
    """Recover an already-paid order whose old Telegram delivery failed."""
    ...
    if len(parts) != 2 or not parts[1].strip():
        return await message.answer(
            "Usage:\n<code>/redeliver ORD-XXXXXXXXXX</code>"
        )
    ...
    if not items:
        return await message.answer(
            "❌ This order has no reserved/delivered product data to resend."
        )

    uid = int(order["telegram_id"])

    try:
        await bot_send_redelivery(message.bot, uid, oid, order, items)
    except Exception:
        logging.exception("Admin redelivery failed order=%s", oid)
        return await message.answer(
            "❌ Redelivery failed. Check Railway logs for the exact Telegram error."
        )

    await message.answer(
        f"✅ Product redelivered to user <code>{uid}</code>\n"
        f"Order: <code>{html.escape(oid)}</code>"
    )
