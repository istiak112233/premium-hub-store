...
def list_users(limit: int = 50):
    with db() as con:
        return con.execute("SELECT * FROM users ORDER BY id DESC LIMIT ?", (limit,)).fetchall()
...
def set_email(uid: int, email: str | None):
    ...
    with db() as con:
        if email:
            ...
            con.execute(
                "UPDATE users SET email=?, email_verified=0, email_verification_token=? WHERE telegram_id=?",
                (email, code, uid),
            )
        else:
            con.execute(
                "UPDATE users SET email=NULL, email_verified=0, email_verification_token=NULL WHERE telegram_id=?",
                (uid,),
            )
        con.commit()
...
def get_email_verification(uid: int):
    with db() as con:
        return con.execute(
            "SELECT email, email_verified, email_verification_token FROM users WHERE telegram_id=?",
            (uid,),
        ).fetchone()
...
def verify_email_code(uid: int, code: str) -> bool:
    """Verify 6-digit code for this user. On success mark email verified and clear the code."""
    code = (code or "").strip()
    if not code.isdigit() or len(code) != 6:
        return False
    with db() as con:
        row = con.execute(
            "SELECT email_verification_token FROM users WHERE telegram_id=?",
            (uid,),
        ).fetchone()
        if not row or str(row["email_verification_token"] or "") != code:
            return False
        con.execute(
...
async def payhub_verify(invoice_id: str, txid: str) -> dict:
    return await payhub_post("/api/v1/verify", {
...
async def payhub_cancel(callback: CallbackQuery, state: FSMContext):
    ...
    with suppress(Exception):
        await callback.message.delete()
    await play_verify_animation(callback.bot, callback.from_user.id, success=False)
    await callback.answer()
...
async def email_received(message: Message, state: FSMContext, bot: Bot):
    ...
    if not sent:
        ...
        return await bot.send_message(
            message.chat.id,
            "⚠️ <b>Email saved, but verification code could not be sent.</b>\n\n"
            "Check SMTP settings in Admin Panel, then try again from Settings.",
...
async def ipn_handler(request: web.Request):
    ...
    if event == "BALANCE_CREDIT":
        try:
            ...
            balance = change_wallet(uid, amount, "DEPOSIT", f"MANUAL-{uuid4().hex[:8].upper()}", note)
            if BOT_INSTANCE:
                await safe_send(
                    BOT_INSTANCE,
                    uid,
    ...
    await apply_paid_invoice(
        BOT_INSTANCE,
        invoice_id,
        str(payload.get("txid") or invoice_id),
        "BINANCE_PAY",
    )
