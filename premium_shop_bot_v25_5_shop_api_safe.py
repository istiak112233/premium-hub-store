# Premium Hub Store Bot - V25.3 Brevo API
# Python 3.10+
# Dependencies:
...
async def sync_shop_api_provider(provider_id: int) -> dict:
    """Safely mirror remote catalog metadata. Existing users/orders/local stock are never deleted."""
    provider = get_api_provider(provider_id)
    if not provider or int(provider['active'] or 0) != 1:
        raise ShopAPIError('API provider not found or disabled.')
    if str(provider.get('provider_type') or 'generic') != 'shop_v1':
        raise ShopAPIError('This provider is not configured as Shop API v1.')

    # Validate the key first. If this fails, do not touch local catalog data.
    ...
    categories = {}
    try:
        cat_data = await shop_api_request(provider, 'GET', '/categories')
        for c in cat_data.get('categories') or []:
            categories[str(c.get('id'))] = str(c.get('name') or 'API Shop')
    except Exception:
        logging.exception('Shop API category sync failed; products will still sync')

    prod_data = await shop_api_request(provider, 'GET', provider['products_endpoint'] or '/products')
    remote_products = prod_data.get('products') or []
    if not isinstance(remote_products, list):
        raise ShopAPIError('Shop API /products returned an invalid list.')

    with db() as con:
        existing_rows = con.execute(
            "SELECT * FROM api_product_map WHERE provider_id=?",
            (provider_id,),
        ).fetchall()
    existing = {str(r['remote_product_id']): r for r in existing_rows}
    ...
    for rp in remote_products:
        if not isinstance(rp, dict) or rp.get('id') is None:
            continue
        remote_id = str(rp.get('id'))
        seen.add(remote_id)
        name = str(rp.get('name') or f'API Product {remote_id}').strip()
        category_id = str(rp.get('category_id') or '')
        category = categories.get(category_id) or 'API Shop'
        description = str(rp.get('description') or '').strip()
        unit_price = Decimal(str(rp.get('unit_price') or rp.get('list_price') or '0'))
        if unit_price < 0:
        ...
        list_price = str(rp.get('list_price') if rp.get('list_price') is not None else unit_price)
        image_url = str(rp.get('image_url') or '')

        mapped = existing.get(remote_id)
        if mapped:
            local_id = int(mapped['local_product_id'])
            with db() as con:
                con.execute(
                    """UPDATE products SET name=?, price=?, validity=?, warranty=?, product_type=?, category=?, note=?, active=1
                       WHERE id=?""",
                    (name, str(unit_price), 'API Delivery', 'Supplier/API', 'API', category, description, local_id),
                )
                con.execute(
                    """UPDATE api_product_map
                       SET remote_category_id=?, remote_stock=?, min_qty=?, max_qty=?, list_price=?, image_url=?, last_sync_at=CURRENT_TIMESTAMP
                       WHERE local_product_id=?""",
                    (category_id, stock, min_qty, max_qty, list_price, image_url, local_id),
                )
                con.commit()
            updated += 1
        else:
            local_id = create_product(name, unit_price, 'API Delivery', 'Supplier/API', 'API', category, description)
            with db() as con:
                con.execute(
                    """INSERT INTO api_product_map(
                           local_product_id,provider_id,remote_product_id,remote_category_id,remote_stock,min_qty,max_qty,list_price,image_url
                       ) VALUES (?,?,?,?,?,?,?,?,?)""",
                    (local_id, provider_id, remote_id, category_id, stock, min_qty, max_qty, list_price, image_url),
                )
                con.commit()
            added += 1

    # /products only returns currently buyable products. Hide missing mapped items, never delete them.
    hidden = 0
    for remote_id, mapped in existing.items():
        if remote_id in seen:
            continue
        with db() as con:
            con.execute("UPDATE products SET active=0 WHERE id=?", (mapped['local_product_id'],))
            con.execute("UPDATE api_product_map SET remote_stock=0, last_sync_at=CURRENT_TIMESTAMP WHERE local_product_id=?", (mapped['local_product_id'],))
            con.commit()
        hidden += 1
    ...
    while True:
        try:
            providers = [
                ...
                if int(p['active'] or 0) == 1 and str(p.get('provider_type') or '') == 'shop_v1'
            ]
...
async def _api_refund_once(bot: Bot, order, provider_id: int, reason: str):
    ...
    await bot.send_message(
        ...
        "⚠️ <b>Supplier order could not be completed</b>\n\n"
...
async def complete_shop_api_order(bot: Bot, order, method: str, mapped) -> None:
    ...
    provider = get_api_provider(provider_id)
    if not provider:
        await _api_refund_once(bot, order, provider_id, 'Supplier API configuration is missing.')
        return
    ...
    if previous:
        ...
        if prev_status == 'FULFILLED' and previous['delivery_json']:
            ...
            await bot.send_message(
                ...
                f"Order <code>{html.escape(oid)}</code> is waiting for supplier confirmation. "
    ...
    if items is None:
        ...
        try:
            ...
            if response.get('ok') is not True:
                raise ShopAPIError(str(response.get('error') or 'Supplier order was not accepted.'), 400, response)
            ...
            await bot.send_message(
                uid,
                "🕒 <b>Payment received — supplier confirmation pending</b>\n\n"
                f"🆔 Order: <code>{html.escape(oid)}</code>\n"
                "The supplier connection returned an uncertain result, so the bot will NOT retry automatically. "
    ...
    if not delivered:
        delivered = "<i>Supplier order fulfilled. No text delivery payload was returned.</i>"
    update_order(
    ...
    remote_link = get_api_order_link(oid)
    remote_code = str(remote_link['remote_order_code'] or '-') if remote_link else '-'
    await bot.send_message(
        uid,
        "✅ <b>Purchase Successful!</b>\n\n"
        "📦 Supplier delivery completed automatically.\n\n"
        f"🆔 Order: <code>{html.escape(oid)}</code>\n"
        f"🔗 Supplier Order: <code>{html.escape(remote_code)}</code>\n"
...
async def admin_api_manager(callback: CallbackQuery, state: FSMContext):
    ...
    lines = ["🔌 <b>API Manager</b>", "", "Connect external suppliers without exposing credentials to customers."]
    for p in providers:
        ptype = str(p.get('provider_type') or 'generic')
    ...
    kb = InlineKeyboardMarkup(inline_keyboard=[
        ...
        [InlineKeyboardButton(text="🔄 Sync Shop API Products", callback_data="admin:shop_api_sync", style="primary")],
...
async def admin_shop_api_connect(callback: CallbackQuery, state: FSMContext):
    ...
    await edit(
        ...
        "🔐 The key is stored privately in the bot database and is never shown to customers.",
...
async def admin_shop_api_config(message: Message, state: FSMContext):
    ...
    await state.clear()
    try:
        result = await sync_shop_api_provider(pid)
        await message.answer(
            "✅ <b>Shop API Connected & Synced</b>\n\n"
            f"Provider ID: <code>{pid}</code>\n"
            f"Products: <b>{result['count']}</b>\n"
            f"Added: <b>{result['added']}</b>\n"
            f"Updated: <b>{result['updated']}</b>\n"
            f"Supplier Balance: <b>{money(result['balance']) if result['balance'] is not None else '-'}</b>\n\n"
            "Existing users, balances, orders and local stock were not deleted.",
            reply_markup=admin_kb(),
        )
    except Exception as exc:
        logging.exception('Initial Shop API sync failed')
        await message.answer(
            f"⚠️ API saved, but initial sync failed: {html.escape(str(exc))}\n\n"
            "No existing shop data was changed. Use API Manager → Sync Shop API Products to retry.",
            reply_markup=admin_kb(),
        )
...
async def admin_shop_api_sync(callback: CallbackQuery):
    if not is_admin(callback.from_user.id): return
    providers = [p for p in list_api_providers() if int(p['active'] or 0) == 1 and str(p.get('provider_type') or '') == 'shop_v1']
    if not providers:
        return await callback.answer('No Shop API v1 provider connected.', show_alert=True)
    await edit(callback, '🔄 <b>Syncing Shop API products...</b>', InlineKeyboardMarkup(inline_keyboard=[[InlineKeyboardButton(text='◀ Back', callback_data='admin:api_manager', style='danger')]]))
    summaries = []
    for p in providers:
        try:
            r = await sync_shop_api_provider(int(p['id']))
            summaries.append(
                f"✅ <b>{html.escape(p['name'])}</b> — {r['count']} products | +{r['added']} new | {r['updated']} updated | {r['hidden']} hidden | Balance: {money(r['balance']) if r['balance'] is not None else '-'}"
            )
        except Exception as exc:
            summaries.append(f"❌ <b>{html.escape(p['name'])}</b> — {html.escape(str(exc))}")
    await edit(
        callback,
        "🔄 <b>Shop API Sync Complete</b>\n\n" + "\n".join(summaries) +
        "\n\n🛡 Existing users, wallet balances, orders and local stock are preserved.",
