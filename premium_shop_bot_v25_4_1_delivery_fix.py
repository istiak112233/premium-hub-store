...
def _delivery_download_kb(oid: str):
    return InlineKeyboardMarkup(inline_keyboard=[
        [InlineKeyboardButton(text="📄 Download Product Details", callback_data=f"delivery:download:{oid}", style="primary")],
...
def _delivery_kb(oid: str, copy_raw: str):
    rows = []
    # Telegram CopyTextButton accepts only short text. Long activation links / account
    # payloads make Telegram reject the whole delivery message with BUTTON_COPY_TEXT_INVALID.
    clean = str(copy_raw or "").replace("\x00", "")
    if 1 <= len(clean) <= 256:
        rows.append([InlineKeyboardButton(text="📋 Copy Product Details", copy_text=CopyTextButton(text=clean), style="primary")])
    else:
        rows.append([InlineKeyboardButton(text="📄 Download Product Details", callback_data=f"delivery:download:{oid}", style="primary")])
    rows.append([InlineKeyboardButton(text="📄 Download Invoice", callback_data=f"invoice:download:{oid}", style="success")])
    rows.append([InlineKeyboardButton(text="🛒 Go to Shop", callback_data="menu:products", style="primary")])
    return InlineKeyboardMarkup(inline_keyboard=rows)


def _plain_saved_delivery(value: str) -> str:
    raw = str(value or "")
    # v25.4 stored HTML-formatted delivery text. Keep old orders recoverable too.
...
async def complete_paid_order(bot: Bot, oid: str, method: str):
    ...
    copy_raw = "\n".join(str(x) for x in items).replace("\x00", "")
    delivered = "\n".join(f"{i}. <code>{html.escape(str(x))}</code>" for i, x in enumerate(items, 1))
    # Persist the payload before sending so the product can always be recovered from My Orders.
    update_order(
        oid,
        payment_method=method,
        status="PAID_DELIVERY_PENDING",
        delivered_content=copy_raw,
    ...
    full_text = header + "🔐 <b>YOUR PRODUCT</b>\n" + delivered + footer
    delivery_kb = _delivery_kb(oid, copy_raw)

    delivered_ok = False
    try:
        if len(full_text) <= 3900:
            await bot.send_message(order["telegram_id"], full_text, reply_markup=delivery_kb)
        else:
            from aiogram.types import BufferedInputFile
            await bot.send_message(
                order["telegram_id"],
                header + "🔐 <b>YOUR PRODUCT</b>\nProduct details are attached as a text file." + footer,
                reply_markup=_delivery_download_kb(oid),
            )
            await bot.send_document(
                order["telegram_id"],
                BufferedInputFile(copy_raw.encode("utf-8"), filename=f"{oid}-product.txt"),
                caption=f"🔐 Product Details • <code>{oid}</code>",
            )
        delivered_ok = True
    except TelegramBadRequest as exc:
        # Final safety fallback: never let a bad CopyText button block delivery.
        logging.warning("Delivery message rejected for %s: %s", oid, exc)
        try:
            fallback_text = full_text if len(full_text) <= 3900 else header + "🔐 <b>YOUR PRODUCT</b>\nUse the download button below." + footer
            await bot.send_message(order["telegram_id"], fallback_text, reply_markup=_delivery_download_kb(oid))
            delivered_ok = True
        except Exception:
            logging.exception("Fallback delivery failed for order %s", oid)
    except Exception:
        logging.exception("Delivery failed for order %s", oid)
    ...
    if delivered_ok:
        ...
        await safe_send(
            ...
            "Payment/stock is safe, but Telegram delivery failed. The customer can recover it from My Orders.",
        )
...
async def my_orders(callback: CallbackQuery):
    ...
    if not rows:
        ...
        for idx, o in enumerate(rows, 1):
            ...
            order_id_raw = str(o["order_id"] or "-")
            ...
            if o.get("delivered_content"):
                action_rows.append([InlineKeyboardButton(
                    text=f"📦 Get Product • {order_id_raw}",
                    callback_data=f"order:delivery:{order_id_raw}",
                    style="success",
                )])
        text = "\n".join(parts)
    action_rows.append([InlineKeyboardButton(text="◀ Back", callback_data="menu:settings", style="danger")])
    await edit(callback, text, InlineKeyboardMarkup(inline_keyboard=action_rows))


@router.callback_query(F.data.startswith("order:delivery:"))
async def order_delivery_recover(callback: CallbackQuery):
    oid = callback.data.split(":", 2)[2]
    order = get_order(oid)
    if not order or (not is_admin(callback.from_user.id) and int(order["telegram_id"]) != callback.from_user.id):
        return await callback.answer("Order not found.", show_alert=True)
    content = _plain_saved_delivery(order.get("delivered_content") or "")
    if not content:
        return await callback.answer("No delivery data is saved for this order.", show_alert=True)
    kb = _delivery_kb(oid, content)
    if len(content) <= 3200:
        await callback.bot.send_message(
            callback.from_user.id,
            "🔐 <b>Your Product</b>\n\n"
        ...
        await callback.bot.send_document(
            callback.from_user.id,
            BufferedInputFile(content.encode("utf-8"), filename=f"{oid}-product.txt"),
            caption=f"🔐 Product Details • <code>{html.escape(oid)}</code>",
            reply_markup=_delivery_download_kb(oid),
        )
    await callback.answer("Product delivery opened.")


@router.callback_query(F.data.startswith("delivery:download:"))
async def download_product_delivery(callback: CallbackQuery):
    oid = callback.data.split(":", 2)[2]
    order = get_order(oid)
    if not order or (not is_admin(callback.from_user.id) and int(order["telegram_id"]) != callback.from_user.id):
        return await callback.answer("Order not found.", show_alert=True)
